...
## Startup / Runtime
- authz-service는 관리자 경로 인가 판정 기준 서비스다.
...
- `curl -i -X POST http://localhost:8084/permissions/internal/admin/verify -H 'X-User-Id: admin-seed' -H 'X-User-Role: ADMIN' -H 'X-Session-Id: session-seed' -H 'X-Original-Method: GET' -H 'X-Original-Path: /v1/admin/blocks' -H 'X-Request-Id: debug-req-1' -H 'X-Correlation-Id: debug-corr-1'`
