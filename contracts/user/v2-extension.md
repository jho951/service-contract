...
이 문서는 `User-server`의 현재 v1 계약을 대체하는 것이 아니라, v2로 확장될 수 있는 축을 정리한다.
...
- capability truth는 `Authz-server`, 공개 여부는 `User-server`, 실제 기능 집행은 소비자 서비스가 담당한다.
