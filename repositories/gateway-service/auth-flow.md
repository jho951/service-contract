...
5. `web` 채널로 판정되면 Gateway는 먼저 `sso_session` 쿠키 존재 여부를 확인하고, 있으면 auth-service 세션 검증 경로로 처리한다.
6. `sso_session`이 없고 `ACCESS_TOKEN`이 있으면 JWT 검증 경로로 처리한다.
7. 검증이 성공하면 Gateway는 내부용 `X-User-Id`, `X-User-Status`, `X-Client-Type`를 재주입하고, `iss=api-gateway`, `aud=internal-services` 계약의 내부용 `Authorization` JWT도 다시 넣는다.
...
- Gateway는 인증 성공 시 `iss=api-gateway`, `aud=internal-services` 계약의 내부용 JWT를 `Authorization`으로 다시 주입한다.
- Gateway는 인증 결과를 내부 컨텍스트로 정규화한다.
- 모든 응답에는 `X-Request-Id`, `X-Correlation-Id`가 포함된다.

## 내부 토큰 패밀리
- 일반 보호 서비스(`user-service`, `editor-service` 등)에는 `iss=api-gateway`, `aud=internal-services` 계약의 내부 JWT를 전달한다.
- `authz-service`의 `/permissions/internal/admin/verify`에는 별도 caller proof 토큰을 전달한다. 기본 issuer는 같아도 audience는 `authz-service`로 좁힌다.
- `authz-service` 토큰은 "이 요청이 Gateway에서 온 내부 판정 요청인가"를 증명하는 용도다.
- 일반 보호 서비스 토큰은 "이 요청이 Gateway 인증을 통과한 사용자 컨텍스트인가"를 전파하는 용도다.
- 두 토큰 패밀리를 섞으면 일반 보호 서비스는 `401 invalid_token`, `authz-service`는 `403 caller proof 실패`로 거부할 수 있다.
